...
let build_precise_fact occ_name (t1,t2) =
  ...
  Pred(Param.event2_pred_block,[FunApp(ev,[occ_name;t1]);FunApp(ev,[occ_name;t2])])
      ...
      let do_end prev_state new_occs new_facts t' =
        ...
        let (new_goal, event_in_goal, success) =
          ...
          update_corresp_goal prev_state.goal (Some (occ,[]))
            (function
              | Pred(pr,[t1';t2']) -> pr == Param.event2_pred && equal_bi_terms_modulo t' (t1',t2')
              | _ -> false
            )
        in
          ...
          begin match fstatus.begin_status with
            ...
            | NoOcc ->
                auto_cleanup_red (fun () ->
                  ...
                  let new_facts = Pred(Param.event2_pred_block,[t1;t2]) :: facts in
		  try
                    find_io_rule (fun _ ->
...
let rec init_rule state tree =
  match tree.desc with
	...
	| Pred(p,_) when p == Param.event2_pred_block -> state
...
let rec extract_conclusion_query restwork = function
  ...
  | QFalse -> raise Unify
  | QConstraints constra -> restwork ([], [], constra, [], [])
...
let rec check_query_falsified_rec restwork event_table constra_prem concl_q evl goall =
  ...
  | QNeq ((t1,_),(t2,_))::rest_evl, _ -> 
      let constra_prem' = Terms.wedge_constraints (Terms.constraints_of_neq t1 t2) constra_prem in
      check_query_falsified_rec restwork event_table constra_prem' concl_q rest_evl goall
  | QGeq ((t1,_),(t2,_))::rest_evl, _ -> 
  ...
  | ev::rest_evl, (Fact(goal,_,_) | EventGoal(goal,_))::rest_goall ->
      let (l,l') =
        ...
        | QSEvent2(t1,t2), Pred(pr,[t1';t2']) when pr == Param.event2_pred -> [t1;t2],[t1';t2']
        | _ ->
      ...
      begin
        try
          TermsEq.unify_modulo_list (fun () ->
            try
              check_query_falsified_rec restwork event_table constra_prem concl_q rest_evl rest_goall
      ...
      begin
        ...
        try
          check_query_falsified_rec (fun () ->
            ...
            false
          ) event_table Terms.true_constraints hyp (List.rev evl) (List.rev goall)
        with Unify -> true
      end
...
let do_reduction bad_reached_in_lemmas opt_query axioms tree =
(*  Profile.start();  *)
  ...
  try
    Reduction_helper.instantiate_natural_predicates (fun () ->
      ...
      let init_state =
        { goal = if bad_reached_in_lemmas then NonInterfGoal (analyze_tree tree) else (build_goal tree query);
...
let do_reduction ?(bad_reached_in_lemmas=false) opt_query axioms tree =
  debug_print "Starting reduction";
  let res =
    Display.auto_cleanup_display (fun () ->
      do_reduction bad_reached_in_lemmas opt_query axioms tree
        )
  in
  Terms.cleanup ();
  res
