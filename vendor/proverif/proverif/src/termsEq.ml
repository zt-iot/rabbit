    ...
    List.iter (function
	  ...
	  if not (List.for_all (fun v -> List.exists (Terms.occurs_var v) l) (!var_list_rhs)) then
	    begin
          ...
          | _ -> user_error ("Does not support equations on non-constructor "^(Display.string_of_fsymb f))
        end
...
let rec copy_remove_syntactic_conclusion_query = function
  | QTrue -> QTrue
  | QFalse -> QFalse
  | QConstraints q -> QConstraints(Terms.map_constraints copy_remove_syntactic q)
