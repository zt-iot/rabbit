...
let rec convert_repl_aux s0 future_copies0 =
  ...
  | None ->
      (* Initial state *)
      assert (List.length future_copies0 = 1);
      let ind_map0 = Array.make 1 0 in
      ind_map0.(0) <- 0;
      (s0, [NoRepl], ind_map0)
  | Some s1 ->
     let future_copies1 =
     ...
     in
     let (s1', rem_copies1, ind_map1) = convert_repl_aux s1 future_copies1 in
     let rem_copies0 =
       match s0.comment with
       | RRepl(n,_) ->
          begin
     ...
     let s0', ind_map0 =
       ...
       | RRepl(n,_) when (List.nth rem_copies1 n != NoRepl) ->
          ...
          s1', ind_map1
       | _ ->
          let ind_map0 = Array.make (List.length future_copies0) 0 in
          let subproc' = copy_processes 0 0 ind_map0 rem_copies0 s0.subprocess in
          { s0 with
            previous_state = Some s1';
            subprocess = subproc';
            goal = update_goal ind_map0 s0.goal;
            comment = update_comment ind_map1 future_copies1 s0.comment },
	  ind_map0
     in
     (s0', rem_copies0, ind_map0)

let convert_repl s0 =
  let future_copies0 = initial_future_copies s0 in
  let (s0', _, _) = convert_repl_aux s0 future_copies0 in
  s0'
