...
let incr_width w = int_of_float ((float_of_int w) *. zoom_factor)
let decr_width w = int_of_float ((float_of_int w) /. zoom_factor)
...
let display_trace_image png_file_name =
  (* Activate the zoom +/- buttons if and only if the size they yield is ok *)
  sets_obj (!b_decr) (decr_width (!png_width) >= min_width);
  sets_obj (!b_incr) (incr_width (!png_width) <= max_width);
  (* Display the trace image *)
  let pixbuf = GdkPixbuf.from_file_at_size png_file_name ~width:!png_width ~height:(height_width_ratio * (!png_width)) in
  ...
  if !Param.trace_win_open = true then
    begin
      ...
      match Sys.command ("dot -Tpng " ^ dot_file_name ^ " -o " ^ png_file_name) with
        0 ->
	  display_trace_image png_file_name
      | _ ->
...
let refresh_img_size () =
  (* Optimized version when only the size of the image changes *)
  let (dot_file_name, png_file_name) = get_file_names() in
  display_trace_image png_file_name
      
(* Generate functions to increase or decrease the size of the png, image of the trace *)
let aux_png next_width_f ()  =
  let new_width = next_width_f (!png_width) in
  if new_width >= min_width && new_width <= max_width then
    begin
      png_width := new_width;
      refresh_img_size ()
    end

let incr_png = aux_png incr_width

let decr_png = aux_png decr_width
...
let display_trace () =
  ...
  let create_button () =
    ...
    let _ = !b_incr#connect#clicked ~callback:(incr_png) in
    let _ = !b_decr#connect#clicked ~callback:(decr_png) in
...
let set_menu_items () =
  ...
  List.iter (fun (i, c) -> ignore(i#connect#activate ~callback:c))
    [(file_select_item, file_selection_window (callback_when_view_exists));
     ...
     (display_trace_item, display_trace);
