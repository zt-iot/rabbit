...
let solve pi_state =
  ...
  (* [encode_destructors] encodes destructors in queries, lemmas 
      and axioms. Only destructors in the conclusion are supported. *)
  let pi_state6 = Encode_queries.encode_destructors pi_state5 in
  (* We display axioms after check_delayed_names (so that names are correctly displayed)
     and after encoding the destructors (so it is clear what is assumed).  *)
  display_axioms pi_state6;
  (* [transl_lemmas] translates lemmas into the needed information
     to apply the lemmas during resolution. *)
  let horn_state2 = Lemma.transl_lemmas horn_state1 pi_state6 in
  ...
  display_query_header Display.TQuery pi_state6;
  ...
  let result =
    ...
    | CorrespQuery _ | CorrespQEnc _ ->
        corresp_solve (horn_state2, pi_state6)
    | _ ->
        [equiv_solve (horn_state2,pi_state6), q]
  in
...
let solve_simplified_process pi_state1 =
  Display.auto_cleanup_display (fun () ->
    ...
    try
	  ...
	  if !Param.prove_lemma_only then
	    begin
	      print_string "\n**Proof of equivalence omitted**\n";
	      store_and_raise_determined true []
	    end
	  else
...
let anal_file s0 =
  if !Arg.current < Array.length Sys.argv - 1 then
    Parsing_helper.user_error "The ProVerif file to analyze should be the last argument.\nIf you want to analyze several files, please run ProVerif once for each file.";
  let s =
  ...
  begin
    ...
    | PiType ->
        ...
        if not !parse_only then display_process pi_state2;

        let pi_state3 = transl_query pi_state2 in
        let pi_state4 = transl_lemma pi_state3 in
  
        if not !parse_only && has_query pi_state4 then
          begin
            if (!Param.html_output) then
              Display.Html.print_string "<UL>\n";
    ...
    | Pi ->
        ...
        decr Param.process_number;
        if not !parse_only then display_process pi_state1;

        let pi_state2 = transl_query pi_state1 in
        if not !parse_only && has_query pi_state2 then
          begin
            if (!Param.html_output) then
              Display.Html.print_string "<UL>\n";
