...
let rec translate_expr ?(ch=false) {Location.data=e; Location.loc=loc} =
  let translate_expr' = function
    ...
    | Syntax.Boolean _b -> error ~loc (UnintendedError "translating boolean")
    | Syntax.String s -> String s
    | Syntax.Integer _z -> error ~loc (UnintendedError "translating Integer")
    | Syntax.Float _f -> error ~loc (UnintendedError "translating float")
    ...
    | Syntax.ParamConst (cid, _e) -> error ~loc (UnintendedError ("translating constant " ^ cid))
