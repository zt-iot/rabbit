...
let leq_secrecy (secrecy_lattice : Cst_syntax.cst_access_policy) (lvl_a : Cst_syntax.secrecy_lvl) (lvl_b : Cst_syntax.secrecy_lvl) = 
  ...
  | Cst_syntax.SNode(a_set), Cst_syntax.SNode(b_set) -> 
    ...
    | LessThanOrEqual -> (raise (TypeException "cannot call leq_secrecy on an integrity lattice"))
    end 
  | _, _ -> false 


(* reads whether integrity lvl a is less than or equal to integrity lvl b *)
let leq_integrity (integrity_lattice : Cst_syntax.cst_access_policy) (lvl_a : Cst_syntax.integrity_lvl) (lvl_b : Cst_syntax.integrity_lvl) = 
  match lvl_a, lvl_b with 
  ...
  | Cst_syntax.INode(a_set), Cst_syntax.INode(b_set) -> 
    ...
    | GreaterThanOrEqual -> 
      (raise (TypeException "cannot call leq_integrity on a secrecy lattice"))
    end
  | _, _ -> false
...
let is_subtype (secrecy_lattice : Cst_syntax.cst_access_policy) (integrity_lattice : Cst_syntax.cst_access_policy) 
  (t1 : Cst_syntax.core_security_ty) (t2 : Cst_syntax.core_security_ty) (loc : Location.t) : bool = 
  ...
  *)
  ...
  let same_datatype = Cst_syntax.equals_datatype t1 t2 loc in 
