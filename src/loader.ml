...
let print_error err ppf =
  ...
  | UnstagedConst x ->
    Format.fprintf ppf "This is unintended error. Contact the developper. Hint: %s" x
;;
...
let rec process_expr ?(param = "") ctx lctx { Location.data = c; Location.loc } =
  let process_expr' ctx lctx = function
    ...
    | Input.Apply (o, el) ->
      ...
      then error ~loc (ForbiddenIdentifier o)
      else if Context.ctx_check_ext_func_and_arity ctx (o, List.length el)
      then Syntax.Apply (o, List.map (fun a -> process_expr ~param ctx lctx a) el)
      else error ~loc (UnknownIdentifier o)
    | Input.Tuple el ->
...
let rec process_expr2 new_meta_vars ctx lctx { Location.data = c; Location.loc } =
  let process_expr2' new_meta_vars ctx lctx = function
    ...
    | Input.Apply (o, el) ->
      ...
      then Syntax.Apply (o, List.map (fun a -> process_expr2 new_meta_vars ctx lctx a) el)
      else error ~loc (UnknownIdentifier o)
    | Input.Tuple el ->
...
let rec process_expr3
          used_const
          used_param_const
          new_meta_vars
          ctx
          lctx
          { Location.data = c; Location.loc }
  =
  let process_expr3' used_const used_param_const new_meta_vars ctx lctx = function
    | Input.Var id ->
      if Context.ctx_check_const ctx id
      then
        if List.exists (fun s -> s = id) used_const
        then Syntax.Const id
        else error ~loc (UnstagedConst id)
      else if Context.ctx_check_ext_const ctx id
      then Syntax.ExtConst id
      else if Context.lctx_check_chan lctx id
      then Syntax.Channel (id, "")
    ...
    | Input.Apply (o, el) ->
      ...
      then
        Syntax.Apply
          ...
          , List.map
              (fun a ->
                 process_expr3 used_const used_param_const new_meta_vars ctx lctx a)
              el )
      else error ~loc (UnknownIdentifier o)
    | Input.Tuple el ->
      Syntax.Tuple
        (List.map
           (fun a -> process_expr3 used_const used_param_const new_meta_vars ctx lctx a)
           el)
    | Input.Param (pid, p) ->
      if Context.ctx_check_param_const ctx pid
      then
        if List.exists (fun s -> s = pid) used_param_const
        then
          Syntax.ParamConst
            (pid, process_expr3 used_const used_param_const new_meta_vars ctx lctx p)
        else error ~loc (UnstagedConst pid)
      else if Context.lctx_check_param_chan lctx pid
      then
        Syntax.ParamChan
          (pid, process_expr3 used_const used_param_const new_meta_vars ctx lctx p)
  ...
  | Input.GlobalFact (id, el) ->
    ...
    , Location.locate
        ...
        (Syntax.GlobalFact
           ( id
           , List.map
               (process_expr3 used_const used_param_const new_meta_vars ctx lctx)
               el )) )
  | _ -> error ~loc (UnknownIdentifier2 "")
;;
...
let process_global_facts_closed used_const used_param_const new_meta_vars ctx lctx fl =
  List.fold_left
    (fun (ctx, fl) f ->
       let ctx, f =
         process_global_fact_closed used_const used_param_const new_meta_vars ctx lctx f
       in
       ctx, fl @ [ f ])
    (ctx, [])
    fl
...
let rec collect_meta_global_fact used_const used_param_const new_meta_vars ctx lctx f =
  ...
  | Error { Location.data = err; Location.loc = locc } ->
    (match err with
     | UnknownVariable v ->
       let r, consts, pconsts, l =
         collect_meta_global_fact
           used_const
           used_param_const
           (v :: new_meta_vars)
           ctx
...
let rec process_cmd ctx lctx { Location.data = c; Location.loc } =
  let process_cmd' ctx lctx = function
       ...
       | Input.Apply (o, args) ->
         ...
         then (
           ...
           then ()
             ...
             error
               ~loc
               (ArgNumMismatch (o, List.length args, Context.lctx_get_func_arity lctx o));
    ...
    | Input.Get (vl, id, fid, c) ->
      ...
      then error ~loc (UnknownIdentifier fid)
      else ();
...
let process_pproc ?(param = "") loc ctx def _pol (proc : Input.pproc) =
  let pid, files, vl, fl, m, cargs, chans, ptype =
    ...
    | Input.ParamProc (pid, param', chans) ->
      ...
      then error ~loc (UnknownIdentifier pid)
      else (
        let Context.
        ...
        (match param'' with
         | Some _ -> ()
         | None -> error ~loc (UnknownIdentifier ""));
    ...
    | Input.Proc (pid, chans) ->
      ...
      then error ~loc (UnknownIdentifier pid)
        ...
        in
        (match param' with
         | Some _ -> error ~loc (UnknownIdentifier "")
         | None -> ());
        let cargs = List.rev cargs in
        let Context.
              { def_proctmpl_files = files
      ...
      List.fold_left2
        (fun (files, vl, fl, m, installed_channels) (is_param, ch_f, ty_f) ch_t ->
           ...
           | Input.ChanArgPlain ch_t ->
             ...
             then error ~loc (UnknownIdentifier ch_t)
             else ();
             let _, chan_ty = List.find (fun (s, _) -> s = ch_t) ctx.Context.ctx_ch in
             if chan_ty = ty_f
             then ()
             else error ~loc (WrongChannelType (ch_f ^ ":" ^ ty_f, ch_t ^ ":" ^ chan_ty));
           ...
           | Input.ChanArgParamInst (cid, e) ->
             ...
             then error ~loc (UnknownIdentifier cid)
             else ();
             let _, chan_ty =
             ...
             if chan_ty = ty_f
             then ()
             else error ~loc (WrongChannelType (ch_f ^ ":" ^ ty_f, cid ^ ":" ^ chan_ty));
             (* replace channel variables and check access policies! *)
...
let process_proc loc ctx def pol (proc : Input.proc) =
  match proc with
  | Input.UnboundedProc p -> [ process_pproc loc ctx def pol p ], []
  | Input.BoundedProc (pname, pl) ->
    let pl = List.map (process_pproc ~param:pname loc ctx def pol) pl in
    [], pl
;;

let rec process_decl ctx pol def sys used_idents used_strings fn ({ Location.data = c; Location.loc } : Input.decl)
  =
  let process_decl' ctx pol def sys used_idents used_strings base_fn = function
    | Input.DeclLoad fn ->
      let new_fn = Filename.dirname base_fn ^ "/" ^ fn in
      let ctx, pol, def, sys, used_idents', used_strings' = load new_fn ctx pol def sys in
      ctx, pol, def, sys, (used_idents' @ used_idents), (used_strings' @ used_strings)
    | Input.DeclExtFun (f, k) ->
      let ctx' =
        if Context.check_used ctx f
        then error ~loc (AlreadyDefined f)
        else if k = 0
        then Context.ctx_add_ext_const ctx f
        else if k > 0
        then Context.ctx_add_ext_func ctx (f, k)
        else error ~loc (NegativeArity k)
      in
      ctx', pol, def, sys, used_idents, used_strings
    | Input.DeclExtEq (e1, e2) ->
      ...
      ( ctx
      , pol
      , Context.def_add_ext_eq def (lctx.Context.lctx_loc_var, e1', e2')
      , sys
      , used_idents, used_strings )
    | Input.DeclExtSyscall (f, typed_args, c) ->
      ...
      let ctx, _lctx, c = process_cmd ctx lctx c in
      ( Context.ctx_add_ext_syscall ctx (f, List.map fst typed_args)
      , pol
      , Context.def_add_ext_syscall def (f, typed_args, c)
      , sys
      , used_idents, used_strings )
    | Input.DeclExtAttack (f, t, typed_args, c) ->
      if Context.check_used ctx f then error ~loc (AlreadyDefined f) else ();
      if not (Context.ctx_check_ext_syscall ctx t)
      then error ~loc (UnknownIdentifier t)
      else ();
      let args_ty = Context.ctx_get_ext_syscall_arity ~loc ctx t in
      if List.length typed_args = List.length args_ty
      then ()
      else error ~loc (ArgNumMismatch (t, List.length typed_args, List.length args_ty));
      (* parse arguments *)
      let lctx =
        List.fold_left
          (fun lctx' ta ->
      ...
      , pol
      , Context.def_add_ext_attack def (f, t, typed_args, c)
      , sys
      , used_idents, used_strings )
    | Input.DeclType (id, c) ->
      if Context.check_used ctx id
      then error ~loc (AlreadyDefined id)
      else Context.ctx_add_ty ctx (id, c), pol, def, sys, used_idents, used_strings
    | Input.DeclAccess (s, t, Some al) ->
      ...
      let f pol' a =
        ...
        | Input.CProc, [ Input.CChan ] ->
          if Context.ctx_check_ext_syscall ctx a
          then Context.pol_add_access pol' (s, t, a)
      ...
      in
      ctx, List.fold_left f pol al, def, sys, used_idents, used_strings
    | Input.DeclAccess (s, t, None) ->
      let tys = Context.ctx_get_ty ~loc ctx s in
      let tyt = List.map (Context.ctx_get_ty ~loc ctx) t in
      let pol =
        match tys, tyt with
        | Input.CProc, [ Input.CChan ] -> Context.pol_add_access_all pol (s, t)
        | Input.CProc, [ Input.CFsys ] -> Context.pol_add_access_all pol (s, t)
        | _, _ -> error ~loc WrongInputType
      in
      ctx, pol, def, sys, used_idents, used_strings
    | Input.DeclAttack (tl, al) ->
      let f t a =
        if Context.ctx_check_ty ctx t
        then
          if Context.ctx_check_ext_attack ctx a
          then
        ...
        else error ~loc (UnknownIdentifier t)
      in
      let p =
        List.fold_left
          (fun p t -> List.fold_left (fun p a -> Context.pol_add_attack p (f t a)) p al)
          pol
          tl
      in
      ctx, p, def, sys, used_idents, used_strings
    | Input.DeclInit (id, e) ->
      if Context.check_used ctx id
      then error ~loc (AlreadyDefined id)
      else (
        let e' =
          match e with
          | Some e -> Some (process_expr ctx Context.lctx_init e)
          | _ -> None
        in
        Context.ctx_add_const ctx id, pol, Context.def_add_const def (id, e'), sys, used_idents, used_strings)
    ...
    | Input.DeclParamInit (id, e) ->
      ...
      then error ~loc (AlreadyDefined id)
        ...
        let e' =
          match e with
          | Some (p, e) -> Some (p, process_expr ~param:p ctx Context.lctx_init e)
          | _ -> None
        in
        ( Context.ctx_add_param_const ctx id
        , pol
        , Context.def_add_param_const def (id, e')
        , sys
        , used_idents, used_strings ))
    | Input.DeclParamChan (id, ty) ->
      if Context.check_used ctx id
      then error ~loc (AlreadyDefined id)
      else Context.ctx_add_param_ch ctx (id, ty), pol, def, sys, used_idents, used_strings
        ...
        let fl' =
          List.map
            (fun (a, e, b) ->
               ...
               | Input.CFsys -> a, process_expr ctx Context.lctx_init e, b
    ...
    | Input.DeclChan (id, c) ->
      if Context.check_used ctx id
      then error ~loc (AlreadyDefined id)
      else Context.ctx_add_ch ctx (id, c), pol, def, sys, used_idents, used_strings
    | Input.DeclParamProc (pid, p, cargs, ty, fls, cl, fs, m) ->
      ...
      let lctx =
        List.fold_left
          (fun lctx' (b, cid, cty) ->
             ...
             else error ~loc (UnknownIdentifier cty))
      ...
      let files =
        List.map
          (fun (path, ty, data) ->
             match Context.ctx_get_ty ~loc ctx ty with
             | Input.CFsys -> process_expr ctx lctx path, ty, process_expr ctx lctx data
             | _ -> error ~loc WrongInputType)
          fls
      in
      ...
      let ctx, lctx, ldef =
        List.fold_left
          (fun (ctx'', lctx'', ldef'') (fid, args, cs) ->
             if Context.lctx_check_func lctx'' fid
             then error ~loc (AlreadyDefined fid)
             else (
               let lctx =
                 List.fold_left
    ...
    | Input.DeclSys (procs, lemmas) ->
      let processed_procs, processed_param_procs =
        List.fold_left
          (fun (pl, parampl) p ->
             let pl', parampl' = process_proc loc ctx def pol p in
             ( pl' @ pl
             , match parampl' with
               | [] -> parampl
               | _ -> parampl' :: parampl ))
                 ...
                 *)
                 (match p.Location.data with
                  ...
                  | Input.Correspondence (a, b) ->
                    (match process_facts ctx Context.lctx_init [ a; b ] with
      ...
      , used_idents, used_strings )
  in
  process_decl' ctx pol def sys used_idents used_strings fn c

and load fn ctx pol def sys =
  let decls, (used_idents, used_strings) = Lexer.read_file Parser.file fn in
  List.fold_left
    (fun (ctx, pol, def, sys, used_idents, used_strings) decl ->
       process_decl ctx pol def sys used_idents used_strings fn decl)
    (ctx, pol, def, sys, used_idents, used_strings)
    decls
;;
