        ...
        (fun (files, vl, fl, m, installed_channels) (is_param, ch_f, ty_f) ch_t ->
           ...
           | Input.ChanArgPlain ch_t ->
               ...
               then ()
                 ...
                 error ~loc (WrongChannelType (ch_f ^ ":" ^ ty_f, ch_t ^ ":" ^ chan_ty));
               ...
               , List.map
                   (fun (fn, args, c) ->
                      fn, args, Substitute.cmd_chan_sub c ch_f new_chan)
                   fl
...
let rec process_decl env fn ({ Location.data = c; Location.loc } : Input.decl) =
  let process_decl' env base_fn = function
    ...
    | Input.DeclExtSyscall (f, typed_args, c) ->
        if Context.check_used env.context f then error ~loc (AlreadyDefined f) else ();
        (* parse arguments *)
        let lctx =
          List.fold_left
            (fun lctx' ta ->
    ...
    | Input.DeclType (id, c) ->
        if Context.check_used env.context id
        then error ~loc (AlreadyDefined id)
        else { env with context = Context.ctx_add_ty env.context (id, c) }
    | Input.DeclAccess (s, t, Some al) ->
        let tys = Context.ctx_get_ty ~loc env.context s in
        let tyt = List.map (Context.ctx_get_ty ~loc env.context) t in
        let f pol' a =
          match tys, tyt with
          | Input.CProc, [ Input.CChan ] ->
              if Context.ctx_check_ext_syscall env.context a
        ...
        in
        { env with access_policy = List.fold_left f env.access_policy al }
    | Input.DeclAccess (s, t, None) ->
        ...
        let pol =
          match tys, tyt with
          | Input.CProc, [ Input.CChan ] ->
              Context.pol_add_access_all env.access_policy (s, t)
          | Input.CProc, [ Input.CFsys ] ->
              Context.pol_add_access_all env.access_policy (s, t)
          | _, _ -> error ~loc WrongInputType
        in
    ...
    | Input.DeclAttack (tl, al) ->
        let f t a =
          if Context.ctx_check_ty env.context t
          then
            if Context.ctx_check_ext_attack env.context a
            then
    ...
    | Input.DeclInit (id, e) ->
        ...
        then error ~loc (AlreadyDefined id)
          ...
          let e' =
            match e with
            | Some e -> Some (process_expr env.context Context.lctx_init e)
            | _ -> None
          in
          { env with
    ...
    | Input.DeclParamInit (id, e) ->
        ...
        then error ~loc (AlreadyDefined id)
          ...
          let e' =
            match e with
            | Some (p, e) ->
                Some (p, process_expr ~param:p env.context Context.lctx_init e)
            | _ -> None
          in
          { env with
    ...
    | Input.DeclParamChan (id, ty) ->
        if Context.check_used env.context id
        then error ~loc (AlreadyDefined id)
        else { env with context = Context.ctx_add_param_ch env.context (id, ty) }
    | Input.DeclFsys (id, fl) ->
        ...
        then error ~loc (AlreadyDefined id)
          ...
          let fl' =
            List.map
              (fun (a, e, b) ->
                 match Context.ctx_get_ty ~loc env.context b with
                 | Input.CFsys -> a, process_expr env.context Context.lctx_init e, b
    ...
    | Input.DeclChan (id, c) ->
        if Context.check_used env.context id
        then error ~loc (AlreadyDefined id)
        else { env with context = Context.ctx_add_ch env.context (id, c) }
    ...
    | Input.DeclSys (procs, lemmas) ->
        let processed_procs, processed_param_procs =
          List.fold_left
            (fun (pl, parampl) p ->
               let pl', parampl' =
                 process_proc loc env.context env.definition env.access_policy p
               in
               ( pl' @ pl
               , match parampl' with
                 | [] -> parampl
                 ...
                 List.fold_left
                   (fun (pl, un) p ->
                      ( { p with
                          Context.proc_pid =
                            List.length
                              (List.filter (fun n -> n = p.Context.proc_name) un)
                        }
                        :: pl
                      , p.Context.proc_name :: un ))
                   ([], [])
