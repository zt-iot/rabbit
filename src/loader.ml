...
type error =
  ...
  | UnknownIdentifier of [`SysCall | `Function | `MetaVar | `Parameter | `LocalVar | `Structure | `Process | `ChannelType | `Type | `Attack] * string
...
let print_error err ppf =
  ...
  | UnknownIdentifier (`SysCall, x) -> Format.fprintf ppf "unknown system call %s" x
  | UnknownIdentifier (`Function, x) -> Format.fprintf ppf "unknown function %s" x
  | UnknownIdentifier (`MetaVar, x) -> Format.fprintf ppf "unknown meta variable %s" x
  | UnknownIdentifier (`Parameter, x) -> Format.fprintf ppf "unknown parameter %s" x
  | UnknownIdentifier (`LocalVar, x) -> Format.fprintf ppf "unknown local variable %s" x
  | UnknownIdentifier (`Structure, x) -> Format.fprintf ppf "unknown structure id %s" x
  | UnknownIdentifier (`Process, x) -> Format.fprintf ppf "unknown process id %s" x
...
let rec process_expr ?(param = "") ctx lctx { Location.data = c; Location.loc } =
  let c = match c with
    ...
    | Input.Apply (o, el) ->
        ...
        if not @@ Context.ctx_check_ext_func_and_arity ctx (o, List.length el) then
          error ~loc (UnknownIdentifier (`Function, o));
...
let rec process_cmd ctx lctx { Location.data = c; Location.loc } =
  let ctx, lctx, c = match c with
    ...
    | Input.Get (vl, id, fid, c) ->
        if not (Context.ctx_check_inj_fact ctx fid) then error ~loc (UnknownIdentifier (`Structure (* ??? *), fid));
...
let process_pproc ?(param = "") loc ctx def _pol (proc : Input.pproc) =
  let pid, files, vl, fl, m, cargs, chans, ptype =
    ...
    | Input.ParamProc (pid, param', chans) ->
        if not (Context.ctx_check_proctmpl ctx pid) then error ~loc (UnknownIdentifier (`Process, pid));
        let Context.
    ...
    List.fold_left2
      (fun (files, vl, fl, m, installed_channels) (is_param, ch_f, ty_f) ch_t ->
         ...
         | Input.ChanArgPlain ch_t ->
             ...
             if not (Context.ctx_check_ch ctx ch_t) then error ~loc (UnknownIdentifier (`ChannelType, ch_t));
             let _, chan_ty = List.find (fun (s, _) -> s = ch_t) ctx.Context.ctx_ch in
             if chan_ty <> ty_f then
         ...
         | Input.ChanArgParam ch_t ->
             ...
             then error ~loc (UnknownIdentifier (`ChannelType, ch_t));
             (* *)
             let _, chan_ty =
         ...
         | Input.ChanArgParamInst (cid, e) ->
             ...
             then error ~loc (UnknownIdentifier (`ChannelType, cid));
             let _, chan_ty =
...
let rec process_decl env fn ({ Location.data = c; Location.loc } : Input.decl) =
  ...
  | Input.DeclExtFun (f, arity) ->
      ...
      let ctx' =
        ...
        if arity = 0 then Context.ctx_add_ext_const env.context f
  ...
  | Input.DeclType (id, tc) ->
      (* [type t : filesys/process/channel] *)
  ...
  | Input.DeclAccess (s, tys, Some syscalls) ->
      ...
      let f pol' syscall =
        ...
        | Input.CProc, [ Input.CChan ] ->
            if Context.ctx_check_ext_syscall env.context syscall
            then Context.pol_add_access pol' (s, tys, syscall)
            else error ~loc (UnknownIdentifier (`SysCall, syscall))
  ...
  | Input.DeclAccess (s, tys, None) ->
      ...
      let pol =
        match tc, tycs with
        | Input.CProc, [ Input.CChan ] ->
            Context.pol_add_access_all env.access_policy (s, tys)
        | Input.CProc, [ Input.CFsys ] ->
            Context.pol_add_access_all env.access_policy (s, tys)
        | _, _ -> error ~loc WrongInputType
      in
  ...
  | Input.DeclAttack (tl, al) ->
      ...
      List.iter (fun t ->
          if not @@ Context.ctx_check_ty env.context t then
            error ~loc (UnknownIdentifier (`Type, t))) tl;
      List.iter (fun a ->
          if not @@ Context.ctx_check_ext_attack env.context a then
            error ~loc (UnknownIdentifier (`Attack, a))) al;
      let p =
        List.fold_left
  ...
  | Input.DeclParamInit (id, e) ->
      ...
      if Context.check_used env.context id then error ~loc (AlreadyDefined id);
      let e' = Option.map (fun (p, e) -> (p, process_expr ~param:p env.context Context.lctx_init e)) e in
      { env with
  ...
  | Input.DeclParamProc (pid, p, chanargs, ty, fls, vars, fs, m) ->
      ...
      let lctx =
        List.fold_left
          (fun lctx' (b, cid, cty) ->
             if not @@ Context.ctx_check_ty_ch ctx cty then
               error ~loc (UnknownIdentifier (`ChannelType, cty));
             if b
      ...
      let files =
        List.map
          (fun (path, ty, data) ->
             if Context.ctx_get_ty ~loc ctx ty <> Input.CFsys then error ~loc WrongInputType;
  ...
  | Input.DeclProc (pid, chanargs, ty, fls, cl, fs, m) ->
